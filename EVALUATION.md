...
### Tốc độ tracker (đo được NGAY — Phase 2, không cần nhãn)

> Đo 2026-06-29, CPU i7-1355U. SimpleTracker stress 50 track/frame, 100 lần đo.

| Tracker | ms/frame (mean) | p95 | Ghi chú |
|---|---|---|---|
| SimpleTracker (IoU + Hungarian) | **4.8 ms** @ 50 tracks | 5.0 ms | demo thật ~3 track → < 0.5 ms; DoD ≤5ms ✅ |
| ByteTrack (ultralytics) | gồm cả inference | — | ~detection-bound (chạy YOLO nội bộ) |

Demo: `output/tracked.mp4` (SimpleTracker, 9 track-id) + `output/bytetracked.mp4`
(ByteTrack, 8 track-id), 300 frame, ~17 FPS. Track id ổn định + trajectory polyline.

### Bảng accuracy tracking (cần nhãn — Phase 8, MOTChallenge/TrackEval)
...
### Tốc độ + robustness (đo NGAY — Phase 3, không cần nhãn)

> 2026-06-29, CPU. ClassicalLaneDetector trên 100 ảnh BDD100K val thật.

| Phương pháp | ms/frame (mean) | p95 | Crash | Ghi chú |
|---|---|---|---|---|
| Classical (HLS+Canny+Hough+polyfit+EMA) | **8.0 ms** | 12.7 ms | 0/100 | demo dashcam: làn trái/phải + ego-area, smoothing ổn định |

### Bảng accuracy lane (cần nhãn — Phase 8: TuSimple/CULane/BDD lane)
...
### Tốc độ + functional (đo NGAY — Phase 4, không cần nhãn)

> 2026-06-29, CPU. HSV classify trên đèn YOLO phát hiện ở ảnh BDD100K val thật.

| Phương pháp | ms/đèn | DoD ≤2ms | Functional |
|---|---|---|---|
| HSV heuristic | **0.10 ms** | ✅ | 250 ảnh: red 35 / yellow 38 / green 18; demo 3 đèn (RED+GREEN+GREEN) phân loại đúng |

> Hạn chế đã biết: đèn nhỏ/xa dễ nhầm (vd green↔yellow). Accuracy có nhãn → Phase 8.

### Bảng accuracy đèn (cần nhãn — Phase 8: LISA/BTTSD)
...
> **Kết quả Phase 6 (2026-06-29):** `tests/test_risk.py` 22 test PASS (TtcEstimator +
> RiskAssessor + DecisionSupport). Scenario head-on: CAUTION→DANGER→**BRAKE**→**STOP**
> (TTC≤0.5s) đúng kỳ vọng. `assess()` **0.086 ms/frame** (DoD <2ms). Demo dashcam 1260
> frame: safe 492 / caution 618 / danger 148; actions brake 142 / stop 6. Đèn đỏ → STOP.
